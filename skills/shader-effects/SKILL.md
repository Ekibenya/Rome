...
This skill is self-contained: all effect functions are inline above. (The upstream repo's SKILL.md listed `references/` and `scripts/` companion files that do not exist in the source repository; those dead pointers were removed when this copy was vendored. For effect ordering, see "Combining Effects" above — distortion first, then color, then vignette, grain last.)
