---
name: svg-schematic-figures
description: Hand-author publication-quality technical schematic figures in clean SVG — flowcharts, structural diagrams, cross-sections, labeled illustrations with dashed leader lines and callout labels in the margins. Strict conventions for canvas layout, two-size typography, semantic color ramps, stroke weights, arrow markers, and spacing. Use for technical diagrams, schematic/blueprint-style illustration, or any SVG figure with leader-line labels pointing at parts.
---

> Source: causify-ai/helpers `.claude/skills/figure.create_svg_from_description`. Note: the `c-*` ramp classes and `t/ts/th` text classes referenced below are conventions — define them in an inline `<style>` block (e.g. `.c-blue { fill:#eff6ff; stroke:#2563eb; }`, `.th { font:500 14px sans-serif; }`) or substitute your project palette.
