---
name: svg-authoring
description: Author production-grade hand-written inline SVG (logos, marks, icons, silhouettes, vector illustrations). Enforces viewBox discipline, path budgets, palette limits, optical balance, silhouette clarity, and small-scale legibility for hand-coded `<svg>`. Use when writing SVG paths by hand, drawing shapes or silhouettes in SVG, or reviewing hand-authored SVG quality.
---

# SVG authoring (hand-written inline SVG)

> Source: MohamedAbdallah-14/prompt-to-asset `.cursor/skills/svg-authoring` — adapted: pipeline tool calls removed. Treat "the brief" as whatever constraints the current task defines (viewBox, palette, path budget).
