...
def score(path):
    ...
    # 7) 世界そのものの説明：概念エンジン「册」を毎回書く。
    #    どの開局から入っても、その一篇だけで世界が分かること。
    #    ローマ版では「神册に名が在る者は死なない／消せば帳尻が一度に返る／
    #    枠は八十一しかない／殺すのは剣ではなく筆」のどれかが本文に要る。
    ROMA_ENGINE = (r'(神册|册上|在册|行册|蜡板)[^。]{0,60}'
                   r'(不死|不会死|死不了|伤会?合|划掉|划了|除籍|八十一|格子|一笔)'
                   r'|(不死|不会死|死不了|伤会?合|划掉|划了|除籍|八十一|格子)[^。]{0,60}'
                   r'(神册|册上|在册|行册|蜡板)')
    if is_opening and not re.search(ROMA_ENGINE, raw):
        struct.append(('C7', '-', '「册」の説明が無い（毎篇に必要）'))
    ...
    CLICHE = ['尸横遍野', '血流成河', '惨绝人寰', '生灵涂炭', '哀鸿遍野', '尸山血海',
              ...
              '惨状', '暴行', '人间地狱']
    for w in (CLICHE if is_opening else []):
        if w in raw:
            struct.append(('C15', w, '暴力を成語で要約している＝読者に想像させない語'))
    # 16) 抽象名詞。語り手が感情に名前を付けた時点で、読者は物を見なくて済む。
    # ※ C15/C16/C19/C22 は依頼者が開局に課した要求であって、コーパスの合否条件ではない。
    #   is_opening で門を付けないと corpus/ko1・ko5・ko7 が落ちる（実測して判明）。
    #   「コーパス7本が落ちる変更は誤り」という自己規定に従い、開局限定に直した。
    for w in (['绝望', '恐惧', '悲惨', '残暴', '恐怖气氛'] if is_opening else []):
        if w in raw:
    ...
    for w in (['这就是战争', '历史无声', '历史记住', '无声地记录', '这就是命',
              '战争就是这样', '人就是这样'] if is_opening else []):
        if w in raw:
